...
For a given assumption about the shedding timecourse and how it varies among infected individuals, there will be some effective size $n_e$, which will be less than the total number of shedders $n$, that captures how the $CV$ decreases.
...
gamma_dists %>%
  ...
  ggplot(aes(x / mu, density, color = cv)) +
  theme(
...
#### MGS monitoring performance is independent of WWTP catchment size and flow rate at large numbers
...
We would therefore need to sequence a single sample to a depth of $10^{8.5}$ fragments to see 10 fragments of SARS2 when $i = 10^{-4}$ of the population is infected.

Consider how many reads we'd need to sequence to see an individual 40-mer in the SARS2 genome 10 times when $i = 10^{-4}$, for 1x150bp sequencing.
Roughly $10^{-2.5}$ of SARS2 reads will cover the 40-mer ($(150 - 40 + 1) / 30000$).
Therefore we need to sequence $10^{10}$ reads.
